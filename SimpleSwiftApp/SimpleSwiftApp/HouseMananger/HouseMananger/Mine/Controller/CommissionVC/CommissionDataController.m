...
@implementation CommissionDataController

- (void)mrCommission:(NSDictionary *)theParams {
    // Tag
    NSInteger iTag					= HTTP_CONTROLLER_COMMISSION_LIST;
    // HTTP
    ...
    [bodyDic setObject:[theParams objectForKey:@"settlement"]   forKey:@"settlement"];
    ...
    [postParam setObject:URL_COMMISSION_LIST    forKey:@"act"];
...
- (void)mrFinishCommission:(NSDictionary *)theParams {
    // Tag
    NSInteger iTag					= HTTP_CONTROLLER_COMMISSION_FINISH_LIST;
    // HTTP
    NSString *url					= URL_BASE_TEST;
    HttpConnMethod httpMethod		= HttpConnMethod_POST;
    NSMutableDictionary	*getParam	= [[NSMutableDictionary alloc] init];
...
- (void)requestFinished:(NSInteger)tag retcode:(NSInteger)retcode receiveData:(NSDictionary *)data {
    ...
    if (HTTP_CONTROLLER_COMMISSION_LIST == tag) {
        
        if ([self.delegate respondsToSelector:@selector(onGetCommissionReceiveData:withReqTag:)]) {
            [self.delegate onGetCommissionReceiveData:data withReqTag:tag];
        }
    }
    
    if (HTTP_CONTROLLER_COMMISSION_FINISH_LIST == tag) {
        
        if ([self.delegate respondsToSelector:@selector(onGetFinishCommissionReceiveData:withReqTag:)]) {
            [self.delegate onGetFinishCommissionReceiveData:data withReqTag:tag];
        }
    }
}
...
- (void)requestFailed:(NSInteger)tag error:(NSError *)error {
    ...
    if (HTTP_CONTROLLER_COMMISSION_LIST == tag) {
        ...
        if ([self.delegate respondsToSelector:@selector(onGetCommissionFailedWithError:withReqTag:)]) {
            [self.delegate onGetCommissionFailedWithError:error withReqTag:tag];
        }
    }
    
    if (HTTP_CONTROLLER_COMMISSION_FINISH_LIST == tag) {
        
        if ([self.delegate respondsToSelector:@selector(onGetFinishCommissionFailedWithError:withReqTag:)]) {
            [self.delegate onGetFinishCommissionFailedWithError:error withReqTag:tag];
        }
    }
}
