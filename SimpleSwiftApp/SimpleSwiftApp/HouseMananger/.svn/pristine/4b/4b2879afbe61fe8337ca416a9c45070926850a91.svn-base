...
FinishListViewControllerDelegate,
WaitingLishViewControllerDelegate
>
{
    FinishListViewController *_finishListVC;
    WaitingLishViewController *_waitingListVC;
}
...
- (void)loadMainView {
    ...
    // 已结算佣金
    _finishListVC = [[FinishListViewController alloc] initWithFrame:CGRectMake(0,
                                                                               0,
                                                                               size_width,
                                                                               size_height - 44 - CONTROL_PANEL_HEIGHT)];
    _finishListVC.delegate = self;
    [self.bgScrollView addSubview:_finishListVC.view];
    
    
    // 待结算佣金
    _waitingListVC = [[WaitingLishViewController alloc] initWithFrame:CGRectMake(size_width,
                                                                                 0,
                                                                                 size_width,
                                                                                 size_height - 44 - CONTROL_PANEL_HEIGHT)];
    _waitingListVC.delegate = self;
    [self.bgScrollView addSubview:_waitingListVC.view];
    
}

#pragma mark - 工具条 已结佣金 待结佣金
