//
//  CommissionDetailDataController.m
//  HouseMananger
//
...
@implementation CommissionDetailDataController

- (void)mrCommissionDetail:(NSDictionary *)theParams {
    // Tag
    NSInteger iTag					= HTTP_CONTROLLER_COMMISSION_DETAIL;
    // HTTP
    ...
    // Cache
    NSString *identifier			= [[NSString alloc] initWithFormat:@"MY_COMMISSION_DETAIL"];
    NSString *path					= [[NSString alloc] initWithFormat:@"API"];
    NSString *fileName				= [[NSString alloc] initWithFormat:@"MY_COMMISSION_DETAIL"];
    // Use cache
    ...
    [postParam setObject:URL_COMMISSION_DETAIL      forKey:@"act"];
...
- (void)requestFinished:(NSInteger)tag retcode:(NSInteger)retcode receiveData:(NSDictionary *)data {
    ...
    if (HTTP_CONTROLLER_COMMISSION_DETAIL == tag) {
        
        if ([self.delegate respondsToSelector:@selector(onGetCommissionDetailReceiveData:withReqTag:)]) {
            [self.delegate onGetCommissionDetailReceiveData:data withReqTag:tag];
        }
    }
}
...
- (void)requestFailed:(NSInteger)tag error:(NSError *)error {
    ...
    if (HTTP_CONTROLLER_COMMISSION_DETAIL == tag) {
        ...
        if ([self.delegate respondsToSelector:@selector(onGetCommissionDetailFailedWithError:withReqTag:)]) {
            [self.delegate onGetCommissionDetailFailedWithError:error withReqTag:tag];
        }
    }
}
